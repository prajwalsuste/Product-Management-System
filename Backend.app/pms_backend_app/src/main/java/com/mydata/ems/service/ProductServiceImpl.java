...
public class ProductServiceImpl implements ProductService {
	...
	public ProductDTO updateProduct(Long id, ProductDTO pd) throws ResourceNotFoundException {
		Product p =proRepo.findById(id)
				...
				p.setProduct_cost(pd.getProduct_cost());
				p.setProduct_qty(pd.getProduct_qty());
				p.setProduct_category(pd.getProduct_category());
				Product updatedPro =proRepo.save(p);
				return ProductMapper.mapToProductDTO(updatedPro);
	}
